...
export class ReportsApiService {
    ...
    public dailyExpenses(calendars: Calendar[], dateFrom: Date, dateTo: Date): Observable<ExpenseReportResponse> {
        return this.reportRequest<ExpenseReportResponse>('daily-expenses', ExpenseReportResponse, calendars, dateFrom, dateTo);
    }

    public monthlyExpenses(calendars: Calendar[], dateFrom: Date, dateTo: Date): Observable<ExpenseReportResponse> {
        return this.reportRequest<ExpenseReportResponse>('monthly-expenses', ExpenseReportResponse, calendars, dateFrom, dateTo);
    }

    public categoryExpenses(calendars: Calendar[], dateFrom: Date, dateTo: Date): Observable<CategoryExpenseReportResponse> {
        return this.reportRequest<CategoryExpenseReportResponse>('category-expenses', CategoryExpenseReportResponse, calendars, dateFrom, dateTo);
    }

    protected reportRequest<T>(reportType: string, responseType: any, calendars: Calendar[], dateFrom: Date, dateTo: Date): Observable<T> {
        ...
        return this.http.get(`${this.backend}/${reportType}/${calendarIDs}/${tsFrom}/${tsTo}`)
            .pipe(
